...
| Radius | `--dp-radius-chip` 4 · `--dp-radius-button` 6 · `--dp-radius-input` 6 · `--dp-radius-panel` 8 · `--dp-radius-dialog` 12 | Per element kind — a card is a panel, a text field an input. |
| Density | `--dp-density-control-height` 30 · `--dp-density-row-padding` 8 · `--dp-density-min-target` 24 | Pointer density. Buttons, inputs and segmented controls stand 30px tall; table and list rows take 8px of vertical padding; no interactive target goes below 24×24. |
...
| Layout | `--dp-layout-content-max` 1120px · `--dp-layout-readable-max` 760px · `--dp-layout-header-height` 56px · `--dp-layout-rail` 280px · `--dp-layout-rail-collapsed` 80px · breakpoints `--dp-breakpoint-medium` 600px · `--dp-breakpoint-expanded` 840px · `--dp-breakpoint-large` 1240px | Compact < 600, Medium 600–839, Expanded 840–1239, Large ≥ 1240 (Material 3 window classes). Body is centred at `--dp-layout-content-max`, reading columns at `--dp-layout-readable-max`. Navigation is a top header of `--dp-layout-header-height`; the rail widths belong to the admin console. |

Accessibility floor (from `guidelines/DESIGN.md` §6): 24×24px pointer targets (WCAG 2.2 AA 2.5.8) with `--dp-density-control-height` 30px as the standard control height, visible focus ring (`--dp-state-focus-ring`), text contrast ≥ 4.5:1 using the token pairs above, 200% text without horizontal overflow, reduced motion honoured.
  ...
  return (
    <section style={{
      ...
      <button onClick={onOpen} style={{
        minHeight: 'var(--dp-density-control-height)', padding: '0 var(--dp-space-lg)', border: 0, borderRadius: 'var(--dp-radius-button)',
