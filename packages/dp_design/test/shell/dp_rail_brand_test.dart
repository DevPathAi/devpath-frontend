...
/// 컴포넌트가 깐 headerText가 진다. 2단계에서 이 결함이 319/319 green으로
/// 통과했다(라이트 textPrimary == headerBg → 대비 1.00:1).
...
void main() {
  ...
  testWidgets('펼침에서 워드마크 실효색이 headerText다 (라이트)', (tester) async {
    ...
    expect(color, DpColors.light.headerText);
  });

  testWidgets('펼침에서 워드마크 실효색이 headerText다 (다크)', (tester) async {
    final theme = DpTheme.dark();
    await tester.pumpWidget(host(DpColors.dark, theme, extended: true));

    final color = effectiveColorOf(tester, find.text('DevPath'));
    // 주의: 다크 팔레트는 textPrimary == headerText(#EAE7E2)라 이 단언은
    // 현재 가드로서 무력하다 — 함정이 재발해도 통과한다. 라이트 분기가
    // 실질 가드다. 다크 팔레트에서 두 값이 갈리면 이 주석을 지워라.
    expect(color, DpColors.dark.headerText);
  });
