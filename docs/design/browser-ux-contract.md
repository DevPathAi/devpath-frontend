...
| `keyboard-traversal` | 1440 | `/community` 에서 Tab 순회 순서가 `expectations.json` 의 실측 고정값과 일치. S3-P2 에서 셸이 상단 헤더가 되어 순회 시작점이 바뀐다 — 기대값은 그 PR 의 CI 실측으로 다시 기록했다 | 순서 불일치 · 기대값 미기록 |
