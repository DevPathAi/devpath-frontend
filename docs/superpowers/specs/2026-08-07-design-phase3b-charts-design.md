...
| 전체 진행률 | `progress_donut.dart:33,39` | 완료 `c.primary` / 미완료 `c.border` | 완료만 `c.chart1`로 바꾼다. **미완료는 `c.border`를 유지한다 — 아래 §4.3 참조** |
...
### 4.3 ★미완료 트랙 색 — 초안의 교체 지시를 철회한다(실측)★

초안은 도넛 미완료를 `c.border`(경계선 토큰) → `c.surfaceMuted`(면 토큰)로 바꾸라고 했다. 「면에는 면 토큰」이라는 의미론은 옳다. **그러나 육안 확인 단계에서 실측하니 가시성이 떨어졌다.**

| 카드(`surface`) 대비 | `surfaceMuted` | `border` |
|---|---|---|
| 라이트 | 1.14:1 | **1.34:1** |
| 다크 | **1.08:1** | **1.32:1** |

다크에서는 축소 스크린샷에서 미완료 조각이 **보이지 않았고** 확대해야 형태가 잡혔다. WCAG 비텍스트 3:1은 「의미 전달에 필요한」 요소에만 적용되므로 위반은 아니지만, **바꾸기 전이 더 잘 보였다.**

→ **미완료 트랙(도넛 조각·주차별 배경 막대)은 `c.border`를 쓴다**(사용자 결정). 의미론보다 실측 가시성을 택한다. 트랙 색을 다시 검토할 때는 `surfaceMuted` 값 자체를 올리는 쪽을 보되, 그 토큰을 쓰는 다른 화면에 파급되므로 전수 재검증이 따라온다.

**교훈:** 「토큰의 의미가 맞는가」와 「그 토큰이 그 배경 위에서 보이는가」는 별개다. 토큰 교체는 대비를 함께 계산해야 한다.
