...
Run:
```bash
cd packages/dp_design && flutter test --update-goldens test/golden/state_golden_test.dart ; cd ../..
```
Expected: `test/golden/goldens/kill_switch_light.png`·`kill_switch_dark.png` 생성. (폰트 미배치 시 폴백 글리프로 생성됨 — 폰트 배치 후 재생성 권장.)

> **CI 주의(리스크)**: 픽셀 골든은 OS/폰트 렌더러 종속. CI(ubuntu)와 로컬(Windows)이 다르면 diff 발생 → 골든은 **CI 기준 플랫폼에서 생성/검증**하거나, 초기엔 `--update-goldens`로 baseline 후 추적. 광범위 골든화는 보류(TODOS 후보).

- [ ] **Step 3: 골든 검증(생성 직후 동일성)**

Run:
```bash
cd packages/dp_design && flutter test test/golden/state_golden_test.dart ; cd ../..
```
...
melos run test
```
Expected: dp_design `flutter analyze` 이슈 없음, 전 테스트 PASS(골든 포함).
