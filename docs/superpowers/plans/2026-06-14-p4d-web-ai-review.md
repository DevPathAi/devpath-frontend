            ...
            for (final i in items)
              Padding(
                ...
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Icon(Icons.circle, size: 8, color: _sevColor(context, i.severity)),
...
```
> 👍👎(`thumb_*`)·`Icons.circle`은 임시 Material Icons — DD3 단일 Symbols 준수 위해 `DpIcons` 이관(후속). 피드백 전송은 프로토 범위 밖(no-op).
...
- **임시 아이콘**: `thumb_*`/`circle` 등 Material Icons → `DpIcons` 이관(dp_design 추가, 후속).
